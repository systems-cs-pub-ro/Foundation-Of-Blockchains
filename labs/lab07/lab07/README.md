...
## Smart contracts security challenge

Smart contracts are an essential component for the Elrond Network blockchain. It consists on a set of endpoints, view functions and private methods that are used to compose a set of rules to fit a various of use cases across multiple domains.

The Elrond smart contracts have at its core the RUST programming language which brings all the built-in security mechanisms that the language provides. Even though the security for most of the smart contracts components are already in place such as memory management at compile time, also this type of software can be prone to bugs and errors.

Starting from the known smart contract already deployed on the Elrond testnet network and with the provided open source code for it, the challenge is to find a way to exploit the smart contract and extract more value from it.
