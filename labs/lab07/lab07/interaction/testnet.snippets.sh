...
WALLET_PEM="~/multiversX/keys/shard0.pem"
...
localMint() {
    ...
    token_decimals=0xFFFFFFFFFFFFFFFFFFFF
...
transferToken() {
    ...
    erdpy --verbose contract call $address --recall-nonce \
        ...
        --arguments $token_identifier $amount \
        --send || return
}
...
addLiquidity() {
    method_name="0x$(echo -n 'addLiquidity' | xxd -p -u | tr -d '\n')"
    user_address="$(erdpy wallet pem-address $WALLET_PEM)"
    ...
    erdpy --verbose contract call $user_address --recall-nonce \
        ...
        --gas-limit=30000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --function="MultiESDTNFTTransfer" --arguments $1 0x2 $2 0x0 $3 $4 0x0 $5 ${method_name} $6 $7 \
        --send || return
}
...
removeLiquidity() {
    ...
    erdpy --verbose contract call $user_address --recall-nonce \
      ...
      --proxy=${PROXY} --chain=${CHAIN_ID} \
      --function="ESDTTransfer" \
      --arguments $1 $lp_token $3 $method_name $4 $5 \
      --send || return
}
